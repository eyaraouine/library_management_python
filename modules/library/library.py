...
class Editor(ModelSQL, ModelView):
    ...
    name = fields.Char('Nom', required=True)
    creation_date = fields.Date('Date de création',
        help='La date à laquelle l\'editeur est crée')
    genres = fields.Many2Many('library.editor-library.genre', 'editor',
        'genre', 'Genres')
    number_of_books = fields.Function(
        fields.Integer('Nombre de libres'),
...
class EditorGenreRelation(ModelSQL):
    ...
    editor = fields.Many2One('library.editor', 'Editeur', required=True,
        ondelete='CASCADE')
...
class Author(ModelSQL, ModelView):
    ...
    birth_date = fields.Date('Date de naissance',
        ...
        depends=['death_date'])
    death_date = fields.Date('Date de dècés',
    ...
    latest_book = fields.Function(
        fields.Many2One('library.book', 'Livre le plus recent',
            states={'invisible': ~Eval('books', False)}),
        'getter_latest_book')
...
class Book(ModelSQL, ModelView):
    ...
    author = fields.Many2One('library.author', 'Auteur', required=True,
        ondelete='CASCADE')
    ...
    genre = fields.Many2One('library.genre', 'Genre', ondelete='RESTRICT',
        ...
        required=False)
    editor = fields.Many2One('library.editor', 'Editeur', ondelete='RESTRICT',
        domain=[If(
    ...
    publishing_date = fields.Date('Date de publication')
    description = fields.Char('Description')
    summary = fields.Text('Resumé')
    cover = fields.Binary('Couverture')
    page_count = fields.Integer('Nombre de pages',
        help='Le nombre de pages du livre')
    edition_stopped = fields.Boolean('Edition arrêtée',
        help='Si coché, ce livre ne sera plus imprimé dans cette version encore une fois')
    number_of_exemplaries = fields.Function(
        fields.Integer('Nombre d\'exemplaires'),
        'getter_number_of_exemplaries')
    latest_exemplary = fields.Function(
        fields.Many2One('library.book.exemplary', 'Exemplary plus récent'),
    ...
    def __setup__(cls):
        ...
        cls._sql_constraints += [
            ('author_title_uniq', Unique(t, t.author, t.title),
                'Les titres  des livres doivent être distincts par auteur!'),
            ]

        """ 
        cls._error_messages.update({
                'invalid_isbn': 'ISBN doit être un nombre',
                'bad_isbn_size': 'ISBN doit avoir 13 chiffres',
                'invalid_isbn_checksum': 'Checksum de l\'ISBN est invalide '
                })
         """
    ...
    def validate(cls, books):
        for book in books:
            ...
            try:
                ...
                raise UserError('ISBN doit être un nombre')
            if len(book.isbn) != 13:
                  raise UserError('ISBN doit avoir 13 chiffres')
            checksum = 0
            for idx, digit in enumerate(book.isbn):
                checksum += int(digit) * (1 if idx % 2 else 3)
            if checksum % 10:
                 raise UserError('Checksum de l\'ISBN est invalide ')
...
class Exemplary(ModelSQL, ModelView):
    ...
    book = fields.Many2One('library.book', 'Livre', ondelete='CASCADE',
        required=True)
    identifier = fields.Char('Identifiant', required=True)
    ...
    def __setup__(cls):
        ...
        cls._sql_constraints += [
            ('identifier_uniq', Unique(t, t.identifier),
                'L\'identifiant de l\'exemplaire doit être unique!'),
            ]
